...
test "$(jq -r '.version' "$EXT/metadata.json")" = "1.0.1"
...
test "$(jq -r '."highlight-apps".type' "$EXT/settings-schema.json")" = "entry"
test "$(jq -r '."highlight-effect".options["Red flash"]' "$EXT/settings-schema.json")" = "red-flash"
